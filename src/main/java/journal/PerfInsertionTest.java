...
public class PerfInsertionTest extends AbstractPerf {
	...
	private String runTimeRawMsg;
	
	private KafkaJournal journal;
	
	{
		runTimeRawMsg = raw_msg;
...
//		raw_msg = sb.toString();
	...
	public long doTest() {
		...
		try {
			...
			for(long i = 0;i < ITERATIONS;i++){
				...
				byte[] content = (count+runTimeRawMsg).getBytes(UTF_8);
				byte[] id = (""+count).getBytes(UTF_8);
	...
	public String getRunTimeRawMsg() {
		return runTimeRawMsg;
	}

	public void setRunTimeRawMsg(String runTimeRawMsg) {
		this.runTimeRawMsg = runTimeRawMsg;
	}

	@Override
	public void prePrint() {
		System.out.println("content length "+runTimeRawMsg.getBytes(UTF_8).length+" in bytes");
	}
	...
	public static void main(String[] args) {
		...
		perfTest.setRunTimeRawMsg(WAF_RAW_MSG);
		perfTest.testImplemention();
		...
		perfTest.setRunTimeRawMsg(sb.toString());
		perfTest.testImplemention();
		
		sb = new StringBuilder(raw_msg);
